...
function combobox_(id,array) {
    ...
    return ( 
           ...
           <div ref={ ref_select  }  className={`border border-sky-500 top-0 left-0 w-full h-24 overflow-y-scroll  bg-slate-100   flex flex-col relative justify-start items-start _shadow mt-1 z-40 `}  > 
...
function combobox_2(id,array) {
  ...
  function  handleChange(e){
    ...
    console.log(e.target.value) ;
      ...
      ReactDOM.unmountComponentAtNode(id_Combox) ;  
    ReactDOM.render(<Combox_2 value={{data : array }}  /> ,id_Combox);
...
<div ref={ ref_select  } style={{  width :  `220px` } }   className={`top-0 left-0 w-full   bg-slate-100  relative  z-40 `}  > 
{ props_array.map(( row, index )=>{  return <div   style={{  display: "table-row"  , } }  className={`whitespace-nowrap  hover:bg-gray-400 hover:bg-opacity-50  `}  >
      
       {


  row.map((cell, j) => { return <div  style={  {width :  `73px`,  whiteSpace : 'nowrap' ,  position: 'relative',   backgroundColor: "white" ,  border: "1px ridge #ccc", display: "table-cell", padding: "1px",} }  > { cell }  </div> })
...
<div id="id_Combox" className={`absolute w-full flex flex-col `}  >  </div>
</div>;
}