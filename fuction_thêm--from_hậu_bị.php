...
if($kiem_tra_bang == "Sổ theo dõi hậu bị")
{
   $so_tai =$_POST["post1"];
...
if($kiem_tra_bang == "Sổ theo dõi hậu bị chết(loại)")
{
 $so_tai =$_POST["post1"];
...
if($cout_2 == 0 )
{
$kiem_tra_loi_1 = "Số tai này chưa có trong danh sách hậu bị" ;
goto b;
}
...
if(( strtotime($date_ngay_chet_loai) - strtotime($ngay_nhap_in_csdl) )  < 0*24*60*60 )
	{
	$kiem_tra_loi_1 = "Ngày chết loại bé hơn ngày nhập heo hậu bị" ;
	goto b;
	}

// sql xem số tai hậu bị này đã phối chưa
$sql_2_ = "Select
    sheet2.`ngay_nhap`,
	 sheet2.`so_tai`,
	  sheet2.`lo_nhap`,
...
Where
	...
	sheet2.trai = '".$trai."'
		And     
	sheet2.`ngay_phoi` IS NULL
	";
$result_2_ = mysqli_query($conn, $sql_2_);
$cout_2_ = mysqli_num_rows($result_2_);
if($cout_2_ == 0 )
{
$kiem_tra_loi_1 = "Số tai này đã phối rồi, bạn phải báo chết(loại) trong phần heo nái" ;
goto b;
}
