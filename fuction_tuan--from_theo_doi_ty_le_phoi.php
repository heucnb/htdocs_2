...
<table id="table1" class="table_nhận_dữ_liệu table41ct2 table_ty_le_phoi" > 
...
LEFT JOIN(Select
         CONCAT(year(sheet1.`ngay phoi`),
				...
				CONCAT(WEEK(sheet1.`ngay phoi`,0)+1,'--',ROUND(sheet1.`so ngay loc`/7,0)+1),
						IF(WEEK(sheet1.`ngay phoi`,0)+1=53 and WEEKDAY('".$day_year_them_1."')= 6 ,
				CONCAT('9-', 53,'--',ROUND(sheet1.`so ngay loc`/7,0)+1) ,
						IF(WEEK(sheet1.`ngay phoi`,0)+1=53,
				CONCAT('9-', 54 ,'--',ROUND(sheet1.`so ngay loc`/7,0)+1),
				CONCAT('9-', WEEK(sheet1.`ngay phoi`,0) +1  ,'--',ROUND(sheet1.`so ngay loc`/7,0)+1)
						) ))  
...
SELECT  *
 FROM (SELECT stt + 1000 as stt_xap_sep, CONCAT('". $year_tru_1."', '-', sheet5.`tuan`) as thang  FROM `sheet5`) as bang5
LEFT JOIN(Select
         CONCAT(year(sheet1.`ngay phoi`),
                '-',
				 IF(WEEK(sheet1.`ngay phoi`,0)<9  ,
				CONCAT(WEEK(sheet1.`ngay phoi`,0)+1,'--',TRUNCATE(sheet1.`so ngay loc`/7,0)+1),
						IF(WEEK(sheet1.`ngay phoi`,0)+1=53 and WEEKDAY('".$day_year_them_1."')= 6 ,
				CONCAT('9-', 53,'--',TRUNCATE(sheet1.`so ngay loc`/7,0)+1) ,
						IF(WEEK(sheet1.`ngay phoi`,0)+1=53,
				CONCAT('9-', 54 ,'--',TRUNCATE(sheet1.`so ngay loc`/7,0)+1),
				CONCAT('9-', WEEK(sheet1.`ngay phoi`,0) +1  ,'--',TRUNCATE(sheet1.`so ngay loc`/7,0)+1)
						) ))  
...
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load('excel_ty_le_phoi_tuan.xlsx');
...
$mien_lay_du_lieu_excel = 'DH2:FI21';
