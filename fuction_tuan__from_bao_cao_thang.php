...
$day_post_bat_dau = date('Y-m-d', strtotime(safeSQL($_POST["post1"]).'-01-01')) ; 
...
$sql_7 = "SELECT  *
 FROM (SELECT stt as stt_xap_sep, CONCAT('". safeSQL($_POST["post1"])."', '-', sheet3.`tuan`) as thang  FROM `sheet3`) as bang5
...
'-',
	...
	thang) as bang1
	ON bang5.thang= bang1.thang	   
	
LEFT JOIN (Select
         CONCAT(
year(sheet1.`ngay de`)  , 
'-',
                        IF(WEEK(sheet1.`ngay de`,0)<9  ,
				WEEK(sheet1.`ngay de`,0)+1,
						IF(WEEK(sheet1.`ngay de`,0)+1=53 and WEEKDAY('".$day_year_them_1."')= 6 ,
				CONCAT('9-', 53) ,
						IF(WEEK(sheet1.`ngay de`,0)+1=53,
				CONCAT('9-', 54),
				CONCAT('9-', WEEK(sheet1.`ngay de`,0) +1)
						) ))
   )  As thang,
     ...
     Group By
      Thang) as bang2
ON bang5.thang= bang2.thang	   
LEFT JOIN (Select
        CONCAT(
year(sheet1.`ngay cai`)  , 
'-',
                        IF(WEEK(sheet1.`ngay cai`,0)<9  ,
				WEEK(sheet1.`ngay cai`,0)+1,
						IF(WEEK(sheet1.`ngay cai`,0)+1=53 and WEEKDAY('".$day_year_them_1."')= 6 ,
				CONCAT('9-', 53) ,
						IF(WEEK(sheet1.`ngay cai`,0)+1=53,
				CONCAT('9-', 54),
				CONCAT('9-', WEEK(sheet1.`ngay cai`,0) +1)
						) ))
   )  As thang,
     ...
     Group By
       Thang) as bang3
ON bang5.thang= bang3.thang	   
LEFT JOIN (Select
         CONCAT(
year(sheet1.`ngay heo con chet`)  , 
'-',
                        IF(WEEK(sheet1.`ngay heo con chet`,0)<9  ,
				WEEK(sheet1.`ngay heo con chet`,0)+1,
						IF(WEEK(sheet1.`ngay heo con chet`,0)+1=53 and WEEKDAY('".$day_year_them_1."')= 6 ,
				CONCAT('9-', 53) ,
						IF(WEEK(sheet1.`ngay heo con chet`,0)+1=53,
				CONCAT('9-', 54),
				CONCAT('9-', WEEK(sheet1.`ngay heo con chet`,0) +1)
						) ))
   )  As thang,
...
SELECT  *
 FROM (SELECT stt + 1000 as stt_xap_sep, CONCAT('". $year_tru_1."', '-', sheet3.`tuan`) as thang  FROM `sheet3`) as bang5
...
'-',
                        IF(WEEK(sheet1.`ngay de`,0)<9  ,
				WEEK(sheet1.`ngay de`,0)+1,
						IF(WEEK(sheet1.`ngay de`,0)+1=53 and WEEKDAY('".$day_post_bat_dau."')= 6 ,
				CONCAT('9-', 53) ,
						IF(WEEK(sheet1.`ngay de`,0)+1=53,
				CONCAT('9-', 54),
				CONCAT('9-', WEEK(sheet1.`ngay de`,0) +1)
...
'-',
                        IF(WEEK(sheet1.`ngay cai`,0)<9  ,
				WEEK(sheet1.`ngay cai`,0)+1,
						IF(WEEK(sheet1.`ngay cai`,0)+1=53 and WEEKDAY('".$day_post_bat_dau."')= 6 ,
				CONCAT('9-', 53) ,
						IF(WEEK(sheet1.`ngay cai`,0)+1=53,
				CONCAT('9-', 54),
				CONCAT('9-', WEEK(sheet1.`ngay cai`,0) +1)
...
'-',
                        IF(WEEK(sheet1.`ngay heo con chet`,0)<9  ,
				WEEK(sheet1.`ngay heo con chet`,0)+1,
						IF(WEEK(sheet1.`ngay heo con chet`,0)+1=53 and WEEKDAY('".$day_post_bat_dau."')= 6 ,
				CONCAT('9-', 53) ,
						IF(WEEK(sheet1.`ngay heo con chet`,0)+1=53,
				CONCAT('9-', 54),
				CONCAT('9-', WEEK(sheet1.`ngay heo con chet`,0) +1)
...
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load('excel_bao_cao_tuan.xlsx');
...
if ($so_o_lay === 0) { echo ("Chưa có dữ liệu");} else {
